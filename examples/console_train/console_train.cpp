// console_train.cpp : 이 파일에는 'main' 함수가 포함됩니다. 거기서 프로그램 실행이 시작되고 종료됩니다.
//
...
void convert_label(const std::string& imagefilename, tiny_dnn::label_t& labels)
{
    std::string strLabel;

    strLabel = imagefilename.substr(0, 2);
...
static void train_lenet(const std::string& data_dir_path,
    ...
    std::vector<std::string> test_images_name;

 
    // load file in folder
    std::string strPath_train = data_dir_path + "\\train";
    std::string strPath_test = data_dir_path + "\\test";

    for (const auto& file : directory_iterator(strPath_train))
    {
        ...
        convert_image(filePath, -1.0, 1.0, 32, 32, image);
        train_images.emplace_back(image);

        tiny_dnn::label_t label;
        convert_label(fileName, label);
        train_labels.emplace_back(label);
    }

    for (const auto& file : directory_iterator(strPath_test))
    {
        std::cout << file.path() << std::endl;
        std::cout << file.path().filename() << std::endl;
        ...
        test_images_name.emplace_back(fileName);

        tiny_dnn::vec_t image;
        convert_image(filePath, -1.0, 1.0, 32, 32, image);
        test_images.emplace_back(image);
    ...
    // 테스트 이미지 결과
    for (int testIndex = 0; testIndex < test_images.size(); testIndex++)
    {
        // recognize
        tiny_dnn::vec_t image = test_images[testIndex];
        auto res = nn.predict(image);
        ...
        for (int index = 0; index < 1; index++)
            std::cout << test_images_name[testIndex] << ":" << scores[index].second << "," << scores[index].first << std::endl;
    }
}
